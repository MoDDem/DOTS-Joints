...
public struct SourceMesh
{
    ...
    private void BuildData()
    {
        ...
        foreach (Vector3 vert in Mesh.vertices)
        {
            ...
            {
                transformed.position = math.mul(rotation, transformed.position);
                transformed.normal = math.mul(rotation, transformed.normal);
            }
            if (math.all(scale != new float3(1,1,1)))
            {
                transformed.position = Vector3.Scale(transformed.position, scale);
                transformed.normal = Vector3.Scale(transformed.normal, scale);
            }
            if (math.all(translation != float3.zero))
            {
    ...
    public override bool Equals(object obj)
    {
        ...
        return Mesh == other.Mesh &&
            math.all(translation == other.translation) &&
